...
(function() {
    ...
    // 全局配置信息
    const settings = {
        RESET_MODE: false, // 默认是断点恢复模式
        NUMBER_OF_PAGES_READ_PER_STARTUP: 5, // 每次运行读取的页面数量
        SECONDS_BEFORE_LAST_RUNTIME: 60*5, // 上次运行的间隔时间，以防止插件重复运行
        spider: 'TM',
        type_id_groups: [1,2,3,7,8,13,16],
        selector: {
    ...
    (async function(){
        ...
        if ((new Date().getTime() - settings.SECONDS_BEFORE_LAST_RUNTIME*1000) < lastRuntime()) {
            console.log('Error(main): 无法正常启动，可能上次启动尚未结束，请等待5分钟。lasttime=', new Date(lastRuntime()).toString());
            return -2;
        }
    ...
    function lastRuntime() {
        ...
        for (let x of GM_listValues()) {
            if (settings.type_id_groups.indexOf(x) != null) {
                if (GM_getValue(x).timestamp > lasttime) lasttime = GM_getValue(x).timestamp;
            }
        }
        return lasttime;
    }
