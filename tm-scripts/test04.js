...
(function() {
    ...
    const active_page_selector = 'a.current'; // 这是当前页面按钮（不可用）的位置，还有一个隐藏的输入框是 <input type="hidden" name="page.currentPage" value="3">
    ...
    const record_count_selector = '#pageid2 > table > tbody > tr > td:nth-last-child(3)'; // 倒数第三个按钮，‘共292,298条数据/14,615页’
    ...
    (async function(){
        ...
        if (BREAKPOINT_MODE) {
            ...
            if (total > status.total) { // 断点以来有新的记录
                console.log('Info: 发现自断点以来的新记录，刷新GM状态数据...');
                updateStatus(type_id, total);
                status = getStatus(type_id);
            }
            if (status.end > 0) { // 尾部还没读完
                const page_no = Math.floor((status.total - status.end) / page_size); // 保险起见，断点所在页面再读一次
                console.log('Info: 自动跳转到断点页面，页码=', page_no);
                gotoPage(page_no);
            } else if (status.start < status.total) { // 头部还没读完
                const page_no = Math.floor((status.total - status.start) / page_size) + 1;
                console.log('Info: 自动跳转到断点页面，页码=', page_no);
                gotoPage(page_no);
            } else { // 头尾都空，没有新数据
    ...
    async function gotoPage(pageNumber){
        ...
        if (Number(x) == pageNumber) {
            console.log('Info: 成功调转到断点页码， 当前页码=', x );
            await sleep(3000);
        }
        else {
            //debugger;
